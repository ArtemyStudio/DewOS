    ...
    void redraw() {
        ...
        for (size_t i = start; i < log.size(); ++i) {
            ...
            if (line.size() > 4 && line[2] == '\xe2') {
                ...
                if (line.find("\xe2\x9c\x97") != std::string::npos) {
                    out(ui::FG_GREY);
...
static int menu(const std::string& title, const std::string& subtitle, const std::vector<std::string>& items) {
    int idx = 0;
...
static std::string prompt(const std::string& title, const std::string& label, bool hidden, bool& ok) {
    ok = true;
    ...
    while (true) {
        ...
        if (k >= 32 && k <= 126) {
            ...
            if (hidden) {
                ...
                char ch = static_cast<char>(k); out(std::string(1, ch)); 
            }
        }
    }
}
...
static bool copy_rootfs(Progress& p) {
    if (access("/rootfs-disk", F_OK) != 0) {
        ...
        const char* script = "cd / && find . -xdev "
            "-not -path './proc/*' -not -path './sys/*' -not -path './dev/*' "
            "-not -path './tmp/*' -not -path './mnt/*' -not -path './run/*' "
            "-not -path './rootfs-disk*' | cpio --quiet -pdm /mnt/target";
...
static bool write_system_files(Progress& p, const Config& cfg) {
    ...
    p.add("Writing /etc/passwd, /etc/shadow ...", false);
    {
        ...
        passwd << cfg.username << ":x:1000:1000:" << cfg.username << ":/home/" << cfg.username << ":/init\n";
...
static bool install_grub(Progress& p, const std::string& disk) {
    ...
    if (efi) {
        ...
        int rc = run_visible({"grub-install", "--target=x86_64-efi", "--efi-directory=/mnt/target/boot/efi",
                              "--boot-directory=/mnt/target/boot", "--removable", "--no-nvram"});
        if (rc != 0) {
    ...
    if (!efi) {
        int rc = run_visible({"grub-install", "--target=i386-pc", "--boot-directory=/mnt/target/boot", disk});
        if (rc != 0) {
...
static bool screen_pick_disk(Config& cfg) {
    ...
    int v = menu("Select target disk", "All data on the chosen disk will be erased.", items);
    if (v < 0 || v == (int)disks.size()) {
        return false;
    }
...
static bool screen_confirm(const Config& cfg) {
    ...
    while (true) {
        ...
        if (buf == "INSTALL" || buf == "Install" || buf == "install") {
            return true;
        }
    }
}
...
static bool screen_install(const Config& cfg) {
    ...
    if (!partition_disk(p, cfg.disk)) {
        return false;
    }
    
    if (!format_partitions(p, cfg.disk)) {
        return false;
    }
    
    if (!mount_target(p, cfg.disk)) {
        return false;
    }
    ...
    if (!write_system_files(p, cfg)) {
        return false;
    }
    
    if (!install_grub(p, cfg.disk)) {
        return false;
    }
...
int main() {
    ...
    while (true) {
        ...
        if (screen_install(cfg)) {
            ...
            menu("Install failed", "Something went wrong. Drop to shell to inspect, then reboot.", {"OK"});
            term.disable_raw();
            out(ui::SHOW_CURSOR);
            run_visible({"sh"});
